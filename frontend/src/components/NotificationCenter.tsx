...
export function NotificationCenter() {
  ...
  return (
    <div className="relative" ref={menuRef}>
      ...
      {/* Dropdown Menu */}
      {isOpen && (
        <div className="absolute right-0 mt-2 w-80 sm:w-96 bg-gray-900 border border-white/10 rounded-2xl shadow-2xl overflow-hidden z-50 animate-fade-in origin-top-right">
          <div className="px-4 py-3 border-b border-white/5 flex justify-between items-center bg-gray-950/50">
            <h3 className="font-semibold text-white">Tareas en Segundo Plano</h3>
            ...
            {tasks.length === 0 ? (
              ...
              <ul className="divide-y divide-white/5">
                {[...tasks].sort((a, b) => b.timestamp - a.timestamp).map((task) => (
                  <li key={task.id} className="p-4 hover:bg-white/[0.02] transition-colors group">
                    <div className="flex gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex justify-between items-start mb-1">
                          <p className="text-sm font-medium text-white truncate pr-2" title={task.title}>
          ...
          {tasks.filter(t => t.status === "completed" || t.status === "error").length > 0 && (
            <div className="p-2 border-t border-white/5 bg-gray-950/30">
              <button 
                onClick={clearCompletedTasks}
                className="w-full py-2 text-xs font-medium text-gray-400 hover:text-white transition-colors rounded-lg hover:bg-white/5"
              >
                Limpiar completadas
              </button>
            </div>
