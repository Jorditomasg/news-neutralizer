...
export default function GeneratedNewsDetailPage() {
  ...
  if (loading) {
    return (
      <div className="max-w-4xl mx-auto py-12 px-6 animate-pulse">
        <div className="h-4 w-24 bg-gray-200 dark:bg-white/10 rounded mb-8 transition-colors"></div>
        <div className="h-10 w-3/4 bg-gray-200 dark:bg-white/10 rounded-lg mb-4 transition-colors"></div>
        <div className="h-6 w-full bg-gray-200 dark:bg-white/10 rounded mb-8 transition-colors"></div>
        <div className="space-y-3">
          <div className="h-4 bg-gray-200 dark:bg-white/10 rounded w-full transition-colors"></div>
          <div className="h-4 bg-gray-200 dark:bg-white/10 rounded w-full transition-colors"></div>
          <div className="h-4 bg-gray-200 dark:bg-white/10 rounded w-5/6 transition-colors"></div>
          <div className="h-4 bg-gray-200 dark:bg-white/10 rounded w-full transition-colors"></div>
        </div>
      </div>
    );
  ...
  const formatBody = (text: string) => {
      ...
      return paragraphs.map((paragraph, index) => {
          ...
          return (
              <p key={index} className="mb-4 text-gray-700 dark:text-gray-300 leading-relaxed text-lg transition-colors">
                  {parts.map((part, i) => {
                      if (part.match(/\[\d+\]/)) {
                          return (
                              <span key={i} className="inline-flex items-center justify-center bg-purple-100 dark:bg-purple-500/20 text-purple-700 dark:text-purple-300 border border-purple-200 dark:border-purple-500/30 font-mono text-xs font-bold rounded px-1.5 mx-0.5 cursor-help transition-colors" title="Referencia a hecho extraído">
                                  {part}
                              </span>
                          );
                      }
